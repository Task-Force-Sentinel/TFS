...
if (isnil "tfs_advanced_fatigue_breathVoice") then {
    ...
    if (_globalSounds) then {
        [5, "Use Global Sounds", [], "fatigue"] call TFS_fnc_diaglog;
    };
...
if (_fatigue > 0.4 && {
    ace_advanced_fatigue_lastBreath > (_fatigue * -10 + 9)
} && {
    !underwater _unit
}) then {
    switch (true) do {
        case (_fatigue < 0.6): {
            ...
            if (_pain > _painSuppress) then {
                _breathSound = ["TFS_Advanced_Fatigue_BreathLow", "TFS_Advanced_Fatigue_BreathinjuredLow"] select ((random 1) > 0.66);
            };
            if (_globalSounds) then {
        ...
        case (_fatigue < 0.85): {
            ...
            if (_pain > _painSuppress) then {
                _breathSound = ["TFS_Advanced_Fatigue_BreathMid", "TFS_Advanced_Fatigue_BreathinjuredMid"] select ((random 1) > 0.66);
            };
            if (_globalSounds) then {
                [_breathSound + format ["_%1_%2", tfs_advanced_fatigue_breathVoice, selectRandom [0, 1, 2, 3, 4]], _unit, AGLtoASL (_unit modeltoWorld (_unit selectionPosition "head")), 60, 1.4, -1, false] call TFS_fnc_playSound3D;
            } else {
        ...
        default {
            ...
            if (_pain > _painSuppress) then {
                _breathSound = ["TFS_Advanced_Fatigue_Breathmax", "TFS_Advanced_Fatigue_Breathinjuredmax"] select ((random 1) > 0.66);
            };
            if (_globalSounds) then {
                [_breathSound + format ["_%1_%2", tfs_advanced_fatigue_breathVoice, selectRandom [0, 1, 2, 3, 4]], _unit, AGLtoASL (_unit modeltoWorld (_unit selectionPosition "head")), 60, 1.4, -1, false] call TFS_fnc_playSound3D;
            } else {
