...
if (_arg isEqualTo "") exitWith {
    systemChat "TFS Error: No argument passed. Command usage: #whisper <player> <message>";
};
...
if (_message isEqualTo "") exitWith {
    systemChat "TFS Error: No message passed. Command usage: #whisper <player> <message>";
};
...
if (!isNull _unit) then {
    ...
    systemChat format ["TFS: Whisper sent to %1", name _unit];
} else {
    systemChat FORMAT_1("TFS Error: No unit found containing %1, or more than one found.", str _message);
};
