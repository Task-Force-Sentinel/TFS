...
/*
 * Name = TFSRHS_chat_fnc_cmndWhisper
 * Author = Freddo
 *
 * Syntaxes:
...
if (_arg isEqualTo "") exitWith {
    systemChat "TFSRHS Error: No argument passed. Command usage: #whisper <player> <message>";
};
...
if (_message isEqualTo "") exitWith {
    systemChat "TFSRHS Error: No message passed. Command usage: #whisper <player> <message>";
};
...
if (!isNull _unit) then {
    ...
    systemChat format ["TFSRHS: Whisper sent to %1", name _unit];
} else {
    systemChat FORMAT_1("TFSRHS Error: No unit found containing %1, or more than one found.", str _message);
};
