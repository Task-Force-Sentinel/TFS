...
if (GVAR(mode) == FOLLOWCAM) then {
    ...
    if (surfaceIsWater _pos) then {
        _pos = getPosASLVisual GVAR(target)
    };
...
if (GVAR(mode) == FREECAM) then {
    ...
    GVAR(freecam_move) = GVAR(freecam_move) vectorMultiply (_delta*60);
    // Ensure that speed is consistent.
    ...
    if (_mX > 0) then {
        GVAR(freecam_move) set [0, _mX - abs (_mX)]
    };
    if (_mY > 0) then {
        GVAR(freecam_move) set [1, _mY - abs (_mY)]
    };
    ...
    if (_mZ < 0) then {
        GVAR(freecam_move) set [2, _mZ + abs (_mZ)]
    };
    if (_mZ > 0) then {
        GVAR(freecam_move) set [2, _mZ - abs (_mZ)]
    };
    ...
    if (_z2 < getTerrainHeightASL [_x, _y]) then {
        _z = 0;
        _tmpPos set[2, 0];
    };
    ...
    if (_zscroll != 0) then {
        ...
        private _dirVector = vectorNormalized (screentoWorld (getMouseposition) vectorDiff getPosVisual GVAR(camera));
        // (vectorDirVisual GVAR(camera)
...
if (GVAR(mode) == FIRSTPERSON) then {
    ...
    if (vehicle GVAR(target) != GVAR(target)) then {
        ...
        if ((assignedvehicleRole GVAR(target) select 0) != "Cargo") then {
            _mode = "gunner"
        };
