...
switch (_type) do {
    case "MouseButtondown": {
        ...
        if (_button == 0 && (GVAR(mode) == FOLLOWCAM || GVAR(mode) == FREECAM) && (ctrlClassname _control == QGVAR(EntityTag) || ctrlClassname _control == QGVAR(groupTag))) then {
            ...
            if (_target isEqualtype grpNull) then {
                private _units = units _target select {
                    alive _x
                };
    ...
    case "MouseZChanged" : {
        ...
        if ((GVAR(modifiers_keys) select 0) && (GVAR(modifiers_keys) select 2)) then {
            if (_value > 0) then {
                GVAR(followcam_fov) = (GVAR(followcam_fov) - 0.05 * _value) max 0.1;
            };
            if (_value < 0) then {
                GVAR(followcam_fov) = (GVAR(followcam_fov) + 0.05 * (abs _value)) min 2.0;
            };
        };
        if(GVAR(mode) == FOLLOWCAM && {
            _x
        } count GVAR(modifiers_keys) <= 0) then {
            GVAR(followcam_zoom) = ((GVAR(followcam_zoom) - ((_value)*GVAR(followcam_zoom)/5)) max 0.1) min 650;
        };
        if(GVAR(mode) == FREECAM && {
            _x
        } count GVAR(modifiers_keys) <= 1) then
        {
