...

{
    ...
    private _render = (GVAR(showgroupMarkers) == 1 || !_isAI) && {
        count _screenPos > 0 && _disttoCam <= _viewDistance
    };
    ...
    // Unit / vehicle tags
    {
        ...
        if (_isVeh) then {
            GVAR(vehicles) pushBackUnique (objectParent _x);
            // for speed reasons.
            _control ctrlShow false;
        } else {
            if (alive _x) then {
...

{
    ...
    if (alive _x && {
        ...
        private _hasplayers = (crew _x findif {
            isplayer _x
        }) >= 0;
        (_control controlsgroupCtrl 2) ctrlShow (_disttoCam <= 300);
        (_control controlsgroupCtrl 3) ctrlShow (_hasplayers && {
            _disttoCam <= 150
        });
...

{
    ...
    if (count _data > 0) then {
        ...
        if (_camPos distance2D _pos > 400) then {
            _fontsize = 0
        };
