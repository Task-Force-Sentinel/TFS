...
switch true do {
    ...
    case (_key == DIK_T && _type == KEYdoWN): {
        ...
        if (GVAR(tracers)) then {
            _message = "Tracers have been toggled on"
        };
        systemChat _message;
    };
    case (_key == DIK_K && _type == KEYdoWN): {
        ...
        if (GVAR(bulletTrails)) then {
            _message = "Bullet trails have been toggled on"
        };
        systemChat _message;
    };
        ...
        if (!isNull getAssignedCuratorlogic player) then {
            ...
            [_pos, _vectorUp, _vectorDir] spawn {
                ...
                (findDisplay 312) displayAddEventHandler ["Unload", {
                    ...
                    GVAR(zeusDir) = getDir curatorCamera; GVAR(zeuspitchBank) = curatorCamera call BIS_fnc_getpitchBank;_this spawn FUNC(zeusUnload);
                }];
                sleep 0.5;
                curatorCamera setPos _pos;
                curatorCamera setvectorDirAndUp [_vectorDir, _vectorUp];
