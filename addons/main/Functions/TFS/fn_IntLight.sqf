...
if (isnil {
    ...
    IL_Balls = false;
    // Show the attach points (requires Debug mode)
};
if (isnil {
    IL_crew_Only
}) then {
    IL_crew_Only = false;
    // Only allow pilot/driver/Co-pilot/gunner/commander (does not include left & right gunners for choppers) to change the light
};
if (isnil {
    IL_Red_On_Always
}) then {
    IL_Red_On_Always = true;
    // When true, when light is turned off then on the light will be reset to red else it will never be reset if false
};
if (isnil {
    IL_action_Night
}) then {
    IL_action_Night = false;
    // Only allow the action to come up during the Dark times
};
/*End Definable*/
/*Start Script*/
IL_loaded = false;
...
IL_config = IL_config + [
    ...
    [["TFS_B_Heli_Transport_01_F_Sand"], false, [
        [ [0, 2, 0], IL_c_red, IL_attenuation, IL_intensity ]
    ], true],
    [["TFS_B_Heli_Transport_01_F_Tropic"], false, [
        [ [0, 2, 0], IL_c_red, IL_attenuation, IL_intensity ]
    ], true],
    [["TFS_B_Heli_Transport_01_F_medic"], false, [
        [ [0, 2, 0], IL_c_red, IL_attenuation, IL_intensity ]
    ], true]
];
...
IL_fnc_attachto = {
    ...
    _pos = _this select 2;
    waitUntil{
        _light setPos (_veh modeltoWorld _pos);
        isNull _light
    };
};
...
IL_fnc_addAction = {
    ...
    IL_action = player addAction [
        ...
        if (IL_Debug) then {
            player sideChat format ['vehicle was chosen because of this classname %1', IL_config select (_veh call IL_fnc_inlist) select 0]
        };
        ",
        "",
        0.999,
        ...
        if ((_veh != player)) then {
                ...
                if (_ison) then {
                    ...
                    if ((cameraView != 'inTERNAL') && ((IL_config select _i) select 1)) then {
                        if (count IL_lights > 0) then {
                            if (IL_Debug) then {
                                player sideChat 'Lights being removed because in third person'
                            };
                            if ((IL_Debug) && (IL_Balls) && !(isnil {
                                _veh getVariable 'il_balls'
                            })) then {
                                {
                                    deletevehicle _x;
                                ...
                                if (IL_Debug) then {
                                    player sideChat format ['Balls being removed because in third person %1', str IL_lights];
                                };
                            };
                            {
                        ...
                        if (count IL_lights == 0) then {
                            if (IL_Debug) then {
                                player sideChat 'lights added, hint displayed for the array';
                                hintSilent str _lights;
                            };
                            {
                        ...
                        if (_timestamp != IL_lastchange) then {
                            ...
                            if (IL_Debug) then {
                                player sideChat format ['light colour changed', str _override];
                            };
                            {
                    ...
                    if (count IL_lights > 0) then {
                        if (IL_Debug) then {
                            player sideChat format ['removing lights because lights are off %1', str IL_lights];
                        };
                        {
                            IL_lights = IL_lights - [_x];
                            deletevehicle _x;
                        ...
                        if ((IL_Debug) && (IL_Balls) && !(isnil {
                            ...
                            _veh setVariable ['il_balls', nil, true];
                            if (IL_Debug) then {
                                player sideChat format ['removing balls because lights are off %1', str IL_lights];
                            };
                        };
                    };
                };
            ...
            if (count IL_lights > 0) then {
                if (IL_Debug) then {
                    player sideChat format ['removing lights because you are not in a vehicle %1', str IL_lights];
                };
                {
                    IL_lights = IL_lights - [_x];
                    deletevehicle _x;
        ...
        if (IL_crew_Only) then {
            _show = (_show && ((_this == driver _target) or (_this == gunner _target) or (_this == commander _target) or (_this in (vehicle _target call IL_fnc_returnturretunits))))
        };
        if (IL_action_Night && sunOrMoon == 1) then {
            _show = (_show && (vehicle _target getVariable [IL_varname, false] ))
        };
        _show
    "];
    if (IL_Debug) then {
    ...
    IL_action2 = player addAction [
        ...
        if (isnil {
            _veh getVariable 'IL_override'
        }) then {
            _lighter set [1, IL_c_green];
            
            if (count _lights > 1 && isnil {
                _lightamount
            }) then {
        ...
        if (!(isnil {
            _veh getVariable 'IL_override'
        }) && (IL_c_green in ((_veh getVariable 'IL_override') select 0))) then {
            player setUserActiontext[IL_action2, IL_Red_text];
        };
        if (isnil {
            _veh getVariable 'IL_override'
        }) then {
            player setUserActiontext[IL_action2, IL_Green_text];
        };
        if ((_veh != player) && (_i > -1)) then {
            _show = ((_i > -1) && ((IL_config select _i) select 3))
        };
        if (IL_crew_Only) then {
            _show = (_show && ((_this == driver _target) or (_this == gunner _target) or (_this == commander _target) or (_this in (vehicle _target call IL_fnc_returnturretunits))))
        };
...
if (IL_eventHandler > -1) then {
    ...
    if (IL_Debug) then {
        player sideChat "IL Respawn EventHandler Removed"
    };
};
...
IL_eventHandler = player addEventHandler ["Respawn", {
    [] spawn IL_fnc_addAction;
    if (IL_Debug) then {
        player sideChat "IL Respawn EventHandler executed!";
    };
}];
if (IL_Debug) then {
    player sideChat format ["IL Respawn EventHandler Added, #%1", IL_eventHandler];
};
