...
_objViewDistValue = if (_modeVar isEqualTo 1) then {_textValue  * (call compile _percentVar) min TFS_Main_ViewDistanceMaxObjectDistance} else {_textValue min TFS_Main_ViewDistanceMaxObjectDistance};  // Check if percentage sync mode is used, if so use a percentage coefficient
