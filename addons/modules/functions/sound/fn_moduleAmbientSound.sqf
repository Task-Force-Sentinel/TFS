 ...
 *	\z\tfsrhs\addons\modules\functions\sound\fn_moduleAmbientSound.sqf
 *	by Ojemineh
 *	
 *	module function
 *	
 ...
 *	
 *	Example:
 *	[] call TFSRHS_fnc_moduleAmbientSound;
 *	
 */
...
switch _mode do {
	...
	case "init": {
		...
		if !((typeName _soundOffset) isEqualType "ARRAY") exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_OffsetNotValid", _soundOffset]], "modules"] call TFSRHS_fnc_log;
		};
		...
		if (_source != "") then {
			...
			if (isNull (missionNamespace getVariable [_source, objNull])) exitWith {
				...
				[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_VarIsUnknown", _source]], "modules"] call TFSRHS_fnc_log;
			};
			...
			if !((typeName _soundSource) isEqualType "OBJECT") exitWith {
				...
				[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_IsNotObject", _soundSource]], "modules"] call TFSRHS_fnc_log;
			};
		...
		if (_soundExist isEqualTo 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_UnknownInCfgSounds", _sound]], "modules"] call TFSRHS_fnc_log;
		};
		...
		if (_distance <= 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_DistanceGreaterZero", _distance]], "modules"] call TFSRHS_fnc_log;
		};
		...
		if ((_maxDistance > 0) && (_maxDistance <= _distance)) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_MaxGreaterDistance", _distance, _maxDistance]], "modules"] call TFSRHS_fnc_log;
		};
		...
		if (_duration <= 0) exitWith {
			...
			[2, "ModuleAmbientSound '%1' - %2", [_logic, format [localize "STR_TFSRHS_ModuleError_DurationNotDefined", _sound]], "modules"] call TFSRHS_fnc_log;
		};
		...
		private _pfhHandler = [{
			...
			if (!alive _logic) exitWith {
				[_source, _sound] call TFSRHS_fnc_stop3dSound;
			...
			if ((CBA_missionTime - _lastTimePlayed) >= _duration) then {
				...
				[_source, _sound, _offset, _distance, _maxDistance, _duration] call TFSRHS_fnc_play3dSound;
