...
switch (TFSRHS_Main_MaydayAccess) do {
	case 0: { _fnc_checkAccess = {false}; };
	case 1: { _fnc_checkAccess = {(_this getVariable ["TFSRHS_Section", -1]) isEqualTo "909 EAW"}; };
	...
	if ((_vehicle isKindOf "Air") && !(_vehicle isKindOf "ParachuteBase")) then {
		...
		if (_vehicle isKindOf "Helicopter") then {
			...
			if (_unit call _fnc_checkAccess) then {
				...
				private _handle = [
					{
						...
						if ([_vehicle] call TFSRHS_fnc_isPilotDown) exitWith {
							...
							if ((round (speed _vehicle) != 0) || (not isTouchingGround _vehicle)) then {
								...
								if (_unit in ([_vehicle] call TFSRHS_fnc_getCopilots)) then {
				...
				_vehicle setVariable ["TFSRHS_MaydayHandle", _handle, false];
			};
		};
...
player addEventHandler ["GetOutMan", {
	...
	_handle = _vehicle getVariable ["TFSRHS_MaydayHandle", -1];
